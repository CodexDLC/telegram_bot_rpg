...
class CharacterModifiersRepo(ICharacterModifiersRepo):
    ...
    async def get_modifiers(self, character_id: int) -> CharacterModifiersDTO | None:
        """
    ...
    async def update_specific_modifiers(self, character_id: int, updates: dict[str, Any]) -> None:
        ...
        stmt = update(CharacterModifiers).where(CharacterModifiers.character_id == character_id).values(**updates)
        ...
        try:
            ...
            log.exception(
                f"Ошибка SQLAlchemy при частичном обновлении модификаторов для character_id={character_id}: {e}"
            )
            raise
