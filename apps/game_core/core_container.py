...
from apps.common.services.redis import (
    AccountManager,
    ArenaManager,
    CombatManager,
    RedisService,
...
if TYPE_CHECKING:
    ...
    from apps.game_core.modules.combat.combat_gateway import CombatGateway
...
class CoreContainer:
    ...
    async def shutdown(self):
        ...
        await self.arq_service.close()  # Закрываем пул ARQ
    ...
    def _get_combat_session_service(self) -> "CombatSessionService":
        ...
        # TurnManager теперь использует ArqService
        turn_manager = CombatTurnManager(self.combat_manager, self.arq_service)
    ...
    def get_combat_gateway(self) -> "CombatGateway":
        """
        Возвращает CombatGateway - единую точку входа в Runtime боя.
        """
        from apps.game_core.modules.combat.combat_gateway import CombatGateway

        session_service = self._get_combat_session_service()
        return CombatGateway(session_service)
