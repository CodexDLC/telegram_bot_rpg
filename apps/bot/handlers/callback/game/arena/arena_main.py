...
@router.callback_query(BotState.arena, ArenaQueueCallback.filter(F.action == "menu_main"))
...
@router.callback_query(BotState.arena, ArenaQueueCallback.filter(F.action == "exit_service"))
