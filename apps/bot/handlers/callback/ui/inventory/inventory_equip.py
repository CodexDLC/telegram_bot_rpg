...
async def inventory_equip_list_handler(
    ...
    if user_id != callback_data.user_id:
        log.warning(f"InventoryEquip | status=access_denied user_id={user_id} callback_user_id={callback_data.user_id}")
    ...
    if not char_id:
        log.error(f"InventoryEquip | status=failed reason='char_id not found in FSM' user_id={user_id}")
    ...
    result_dto = await orchestrator.get_item_list(
        char_id, user_id, "equip", callback_data.category, callback_data.page, state_data
    )
    ...
    if result_dto.content and (coords := orchestrator.get_content_coords(state_data, user_id)):
        ...
        await call.answer()
        log.info(
            f"InventoryEquip | event=list_rendered user_id={user_id} char_id={char_id} category='{callback_data.category}' page={callback_data.page}"
        )
    else:
        log.error(f"InventoryEquip | status=failed reason='message_content not found' user_id={user_id}")
