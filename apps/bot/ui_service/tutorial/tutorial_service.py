...
from apps.bot.resources.texts.buttons_callback import Buttons  # Новый импорт
...
class TutorialServiceStats:
    ...
    def get_restart_stats(self) -> tuple[str, InlineKeyboardMarkup]:
        ...
        kb = self._tutorial_kb(Buttons.TUTORIAL_START_BUTTON)  # ИЗМЕНЕНО
        return text, kb
    ...
    def get_next_step(self) -> tuple[str, InlineKeyboardMarkup] | None:
        ...
        try:
            ...
            log.debug(f"Извлечено событие #{self.sim_text_count} для char_id={self.char_id}.")
    ...
    def add_bonus(self, choice_key: str) -> None:
        ...
        if bonus:
            ...
            log.debug(f"Для char_id={self.char_id} добавлен бонус {bonus}.")
        else:
            log.warning(f"Не найден бонус для ключа '{choice_key}'.")
    ...
    def _get_event_pool(self) -> list[dict]:
        ...
        return random.sample(TutorialEventsData.TUTORIAL_EVENT_POOL, self.pool_size)
    ...
    async def finalize_stats(self, session: AsyncSession) -> tuple[str, InlineKeyboardMarkup]:
        ...
        if final_stats_obj:
            ...
            log.error(f"Не удалось финализировать статы для char_id={self.char_id}.")
    ...
    async def _finalize_stats_in_db(self, session: AsyncSession) -> CharacterStatsReadDTO | None:
        ...
        try:
            ...
            log.exception(f"Ошибка во время финализации статов для char_id={self.char_id}: {e}")
