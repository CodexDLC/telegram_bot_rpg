# 💾 Combat Data Service

[⬅️ Назад: Engine](./README.md)

---

## 🎯 Описание
**Location:** `apps/game_core/modules/combat/combat_engine/combat_data_service.py`

**CombatDataService** — это высокоуровневый адаптер данных (Data Mapper).
Он связывает низкоуровневый `CombatManager` (Redis) с бизнес-логикой движка (`Executor`, `Collector`).

**Главная задача:** Превращать сырые JSON/Hash данные из Redis в богатые доменные объекты (`BattleContext`, `ActorSnapshot`) и обратно.

---

## ⚙️ Роли и Интерфейсы

### 1. Для Коллектора (Lightweight Access)
Коллектору не нужен весь контекст боя. Ему нужны только мета-данные и намерения игроков, чтобы собрать пары.

*   **`get_battle_meta(session_id)`**
    *   Загружает `:meta` (статус, шаг, списки команд).
    *   Возвращает `BattleMeta`.
*   **`get_intent_moves(session_id, char_ids)`**
    *   Загружает только намерения (`:moves`) для списка игроков.
    *   Используется для проверки, кто уже походил.
*   **`push_actions_to_queue`**
    *   Записывает сформированные пары (`CombatAction`) в очередь `q:actions`.

### 2. Для Экзекутора (Heavyweight Access)
Экзекутору нужен **полный** контекст для проведения расчетов.

*   **`load_battle_context(session_id)`**
    *   Загружает **ВСЕ** данные сессии (Meta, Actors, Moves).
    *   Использует `CombatManager.load_full_context_data` (Batch Load).
    *   Собирает объекты `ActorSnapshot` (парсит JSON, объединяет Raw, State и Statuses).
    *   Возвращает `BattleContext`.
*   **`commit_session(ctx)`**
    *   Сохраняет изменения обратно в Redis.
    *   Сериализует `ActorSnapshot` обратно в JSON.
    *   Сохраняет логи боя.
    *   Очищает очередь задач.

---

## 🔄 Data Mapping (Преобразование)

### Чтение (Redis -> Domain)
1.  **Raw Data:** Получает словарь `{state: {...}, raw: {...}, meta: {...}, statuses: {...}}` из Redis.
2.  **Assembly:**
    *   Создает `ActorMetaDTO` (объединяя статические данные из `meta` и динамические из `state`).
    *   Создает `ActorRawDTO` (Attributes, Modifiers).
    *   Создает `ActorLoadoutDTO`.
    *   Создает `ActorStatusesDTO` (Abilities + Effects).
3.  **Result:** Возвращает `ActorSnapshot`.

### Запись (Domain -> Redis)
1.  **Extraction:** Извлекает из `ActorSnapshot` только измененные данные.
    *   `state` (HP, En, Tokens).
    *   `statuses` (контейнер с abilities и effects).
    *   `xp_buffer` (накопленный опыт).
2.  **Commit:** Отправляет обновления в `CombatManager` для записи через `JSON.MERGE` или `JSON.SET`.

---

## 🔗 Зависимости
*   **Uses:** `CombatManager` (Infrastructure Layer).
*   **Used By:** `CombatCollector`, `CombatExecutor`.
*   **Returns:** `BattleContext`, `BattleMeta`.
