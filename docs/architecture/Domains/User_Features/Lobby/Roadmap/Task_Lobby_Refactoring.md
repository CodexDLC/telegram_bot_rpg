...
> **Задача:** Расширить документацию на основе текущего кода и создать полноценную задачу по переходу на **Session Mode**.
> **Важно:** Получение данных для сессии должно происходить через `Context System` (Context Assembler).
...
1.  **Создать `SessionManager` (Фасад)**:
    ...
    *   **Update:** Использовать `Context System` для агрегации этих данных.
